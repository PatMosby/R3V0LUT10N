...
/**
 * A PDF printer for book tags (machine-readable sticker on books).
 *
 * @author koschke
 *
 */
public class BookTagPrinter extends Printer {
    ...
    /**
     * Width of an book tag (format A9).
     */
    private final static int width  = mmToPt(52.0f);
    /**
     * Height of an book tag (format A9).
     */
			...
			int columnNumber, int rowNumber, Content content) throws IOException
	{
        final int leftX  = 2;
        final int leftY  = 2;
		printBookTag(document, page, leftX + (columnNumber % 4) * (width + CardSeparation), leftY + rowNumber * height, (BookTagContent)content);
	}

	/**
	 * Prints the book tag at coordinate (x, y).
	 *
	 * @param document document where to print the book tag
	 * @param page page on which to print the book tag
	 * @param x x coordinate
	 * @param y y coordinate
	 * @param content book tag content to be printed
	 * @throws IOException thrown in case of IO errors
	 */
	private void printBookTag(PDDocument document, PDPage page, int x, int y, BookTagContent content) throws IOException {
